...
在实际开发中,每个服务都需要大量的配置文件,因此就需要有一个统一管理配置文件的组件,spring cloud的Config组件/阿里的Diamond组件/携程的Apollo组件等等. 这些组件实现的功能大体相同. 以spring cloud Config来阐述一下服务配置统一管理的工作流程:
...
- spring cloud常用组件实战
    ...
    - eureka组件
        ...
        - 服务续约
            Eureka Client在默认的情况下每隔三十秒向server发一次心跳，证明自己还活着。如果server90秒没有收到心跳则会把该服务移出列表。这个时间间隔是可以更改的，但是官方并不建议这么做。同时隔30秒client就会向server查询服务注册表。
        - 服务下线
    ...
    - Ribbon组件
        ...
        - Ribbon的两种使用方式
            - Ribbon + RestTemplate结合
                ...
                RestTemplate加上@LoadBalanced注解之后,在远程调度时能够负载均衡主要是维护了一个被该注解注解的RestTemplate列表,并给该列表中的对象添加了拦截器,在拦截器的方法中,讲远程调度的方法交给了Ribbon的负载均衡器(LoadBalancerClient)去处理,从而达到负载均衡的目的
